...
class SecurityTest extends TestCase
{
    ...
    {
        return User::where('email', 'admin@mptvi.com')->firstOrFail();
    }
    ...
    {
        $this->post('/login', ['email' => 'admin@mptvi.com', 'password' => 'wrong-password'])
            ->assertSessionHasErrors('email');

        $this->assertDatabaseHas('security_events', ['type' => 'login_failed', 'email' => 'admin@mptvi.com']);
    }

    public function test_successful_login_is_logged(): void
    {
        $this->post('/login', ['email' => 'admin@mptvi.com', 'password' => 'password'])
    ...
    public function test_security_settings_page_is_admin_only(): void
    {
        ...
        $cashier = User::where('email', 'cashier@mptvi.com')->firstOrFail();

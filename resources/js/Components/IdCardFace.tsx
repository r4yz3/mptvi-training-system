...
export function fmtIdDate(d: string | null) {
    if (!d) return null;
    const dt = new Date(d + 'T00:00:00');
    return isNaN(dt.getTime()) ? d : dt.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' });
}

function addressOf(a: IdCardData) {
    return [
        a.barangay ? `Brgy. ${a.barangay}` : null,
        'Magsaysay',
        a.province ?? 'Davao del Sur',
    ].filter(Boolean).join(', ');
}

/** Front face — CR80 portrait. The `.id-card` size is set by the host page's CSS. */
export function IdCardFront({ a, signatory }: { a: IdCardData; signatory: Signatory }) {
    const issued = fmtIdDate(a.issued);
    return (
        <div className="id-card relative flex flex-col overflow-hidden rounded-lg border border-slate-200 bg-white shadow-lg ring-1 ring-black/5 print:shadow-none">
            ...
            <div className="flex flex-1 flex-col items-center justify-between px-3 py-2">
                {a.photo_url
                    ? <img src={a.photo_url} alt="" className="h-[72px] w-[72px] rounded-md border-2 border-white object-cover shadow ring-1 ring-slate-200" />
                    : <UserCircle2 className="h-[72px] w-[72px] text-slate-200" />}

                <div className="text-center">
                    <div className="text-[10px] font-extrabold uppercase leading-tight text-slate-800">{a.name}</div>
                    <div className="mt-0.5 inline-block rounded-full bg-brand-50 px-1.5 py-px text-[6px] font-medium text-brand-700">
                        {a.program ?? '—'}{a.level ? ` · ${a.level}` : ''}
                    </div>
                </div>
                ...
                <div className="flex w-full items-end justify-between">
                    ...
                    <div className="rounded border border-slate-100 bg-white p-0.5 shadow-sm">
                        <QRCodeSVG value={String(a.id)} size={42} level="M" />
                    </div>
                </div>
...
/** Back face — CR80 portrait. */
export function IdCardBack({ a }: { a: IdCardData }) {
    const issued = fmtIdDate(a.issued);
    return (
        <div className="id-card flex flex-col overflow-hidden rounded-lg border border-slate-200 bg-white shadow-lg ring-1 ring-black/5 print:shadow-none">
            ...
            <div className="flex flex-1 flex-col px-3 py-2 text-[7px]">
                <div className="space-y-1">
                    <BackRow label="Address" value={addressOf(a)} />
                    <BackRow label="Contact no." value={a.contact} />
                    <BackRow label="Emergency contact" value={a.emergency_name} />
                    <BackRow label="Emergency no." value={a.emergency_contact} />
                    <BackRow label="Date issued" value={issued} />
                </div>

                <div className="mt-1.5 rounded bg-brand-50 px-2 py-0.5 text-center text-[6px] font-semibold text-brand-700">
                    Valid for S.Y. {a.school_year ?? '—'}
                </div>
